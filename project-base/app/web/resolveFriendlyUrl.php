...
function returnSlug(string $route, ?string $redirectTo): array
{
...
try {
    ...
    $statement = $connection->prepare('SELECT slug FROM friendly_urls WHERE entity_id = :entity_id AND domain_id = :domain_id AND main = true AND route_name = :route_name');
    ...
    returnSlug($result['route_name'], '/' . $mainFriendlyUrl['slug']);
