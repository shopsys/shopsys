...
export default defineConfig({
    server: {
        ...
        port: 35729,
        // the admin bundles are ES modules and therefore cross-origin requests from every configured domain
        // (127.0.0.1, 127.0.0.2, ...), Rsbuild's default CORS allowlist covers localhost and 127.0.0.1 only
        cors: {
            origin: [/^https?:\/\/(localhost|127\.0\.0\.\d+)(:\d+)?$/],
        },
    },
    source: {
        entry: {
    ...
    plugins: [
        ...
        Symfony({
            ...
            // Reprise prepends the prefix verbatim to the logical asset name, so it has to carry the trailing slash
