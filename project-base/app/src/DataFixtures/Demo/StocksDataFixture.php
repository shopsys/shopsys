...
class StocksDataFixture extends AbstractReferenceFixture
{
    ...
    private const string ATTR_IS_DEFAULT_BY_DOMAIN = 'isDefaultByDomain';
    ...
    protected function initStockData(array $demoRow): StockData
    {
        ...
        $stockData->isDefaultByDomain = $demoRow[self::ATTR_IS_DEFAULT_BY_DOMAIN] + $stockData->isDefaultByDomain;
    ...
    {
        $enabledOnFirstDomainOnly = $this->getDomainBoolArray(true);
        $enabledOnAllDomainsExceptFirst = $this->getDomainBoolArray(false);
        $defaultOnNoDomain = [];
        $defaultOnFirstDomainOnly = $this->getDomainBoolArray(true);
        $defaultOnAllDomainsExceptFirst = $this->getDomainBoolArray(false);

        return [
            [
                self::ATTR_NAME => 'Praha - Stodůlky',
                self::ATTR_EXTERNAL => '701-cz',
                self::ATTR_IS_DEFAULT_BY_DOMAIN => $defaultOnNoDomain,
            ...
            [
                ...
                self::ATTR_IS_DEFAULT_BY_DOMAIN => $defaultOnFirstDomainOnly,
                self::ATTR_NOTE => 'Update data in IS after goods are issued',
                self::ATTR_ENABLED_BY_DOMAIN => $enabledOnFirstDomainOnly,
            ],
            [
            ...
            [
                ...
                self::ATTR_IS_DEFAULT_BY_DOMAIN => $defaultOnAllDomainsExceptFirst,
                self::ATTR_NOTE => null,
