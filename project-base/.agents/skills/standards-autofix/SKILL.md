...
- `pnpm run check` runs `tsc --noEmit` and `biome check` concurrently, so both tools' output is interleaved and prefixed with `[0]` / `[1]`. tsc reports `file(line,col): error TSxxxx: message`; biome reports `file:line:col lint/<group>/<rule>` followed by a diff-like suggestion — biome's suggested fix is the fix to make, it is authoritative for formatting and import ordering.
