...
BASIC_LICENSES="0BSD,Apache-2.0,Artistic-1.0,BlueOak-1.0.0,BSD*,BSD-2-Clause,BSD-3-Clause,CC-BY-3.0,CC-BY-4.0,CC0-1.0,ISC,MIT,MIT*,MIT-0,Python-2.0,UNLICENSED,WTFPL"
...
if [ -n "$npm_violations" ]; then
    echo ""
    echo "❌ PNPM packages with non-approved licenses (review BASIC_LICENSES and GPL_LICENSES variables in both check-licenses.sh files if you want to allow it):"
    printf "%s\n" "$npm_violations" | sort -u | sed 's/^/  - /'
    echo ""
    exit 1
fi

echo "✅ PNPM: All dependencies use approved licenses"
exit 0
