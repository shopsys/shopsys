...
/**
 * Text of the HTML without the tags, with decoded entities and collapsed whitespace. The tags are stripped once more
 * after decoding the entities, so that an encoded tag (`&lt;b&gt;`) cannot come back as markup.
 */
export const htmlToPlainText = (html: string | null | undefined): string => {
    ...
    return stripTags(decodeHtmlEntities(stripTags(html)))
        .replace(/\s+/g, ' ')
        .trim();
};
