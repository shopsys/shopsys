...
export const CartListItem: FC<CartListItemProps> = ({
    ...
    const { selectedServiceUuids, onToggleService, isSettingAdditionalServices } = useProductAdditionalServices({
        productUuid: product.uuid,
        gtmProductListName: GtmProductListNameType.cart,
        knownCartItem: type === TypeCartItemTypeEnum.Product ? item : undefined,
    });
    ...
    const productIdentityContent = (
        <>
            ...
            <h3
                className="w-fit max-w-full font-secondary font-semibold text-sm group-hover/product-link:underline lg:text-sm"
    ...
    return (
        <li>
            <section
                ...
                className="relative vl:grid vl:grid-cols-[minmax(26rem,1fr)_8.75rem_minmax(7rem,8rem)_minmax(7rem,max-content)_1.5rem] items-center vl:gap-x-4 xxl:gap-x-10 rounded-xl bg-background-more p-4 vl:p-5 xl:grid-cols-[minmax(32rem,1fr)_8.75rem_minmax(8rem,10rem)_minmax(8rem,max-content)_1.5rem] xl:gap-x-8"
            ...
            >
                ...
                <div className="relative flex vl:contents flex-row flex-wrap items-center justify-between gap-4">
                    {isProductGift && <GiftBadge />}

                    <div className="vl:col-start-1 grid basis-full grid-cols-[5rem_minmax(0,1fr)] vl:grid-cols-[5rem_minmax(8rem,1fr)_1rem_10rem] vl:items-center gap-x-2.5 pt-0 pr-10 vl:pr-0 xl:grid-cols-[5rem_minmax(0,1fr)_2.5rem_13rem]">
                        <div className="relative col-span-2 flex min-h-20 flex-col justify-center gap-2 pl-22.5">
                            {hasProductDetailLink ? (
                                <ExtendedNextLink
                                    preventRedirectOnTextSelection
                                    className="group/product-link w-fit max-w-full select-text text-text-default no-underline hover:text-text-default hover:no-underline focus-visible:outline-hidden"
                        ...
                        {isCartItemPartiallyAvailable(product, quantity) ? (
                            ...
                            <ProductAvailability
                                ...
                                className="col-start-2 vl:col-start-4 vl:row-span-3 vl:row-start-1 vl:w-40 min-w-0 vl:self-center xl:w-52"
                                isInquiryType={product.isInquiryType}
                            />
                        )}
                    </div>

                    <div className="flex vl:contents flex-col justify-between gap-2">
                        {isProduct ? (
                            <Spinbox
                                ...
                                ref={spinboxRef}
                                className="vl:col-start-2 vl:w-35 w-40"
                                step={1}
                            ...
                            <div className="vl:col-start-2 min-w-35 text-center">{quantity}</div>
                        )}

                        {isProduct && isPriceVisible(product.price.priceWithVat) && (
                            <div className="vl:col-start-3 whitespace-nowrap font-secondary xl:w-40">
                    ...
                    <IconButton
                        Icon={TrashCanIcon}
                        className="vl:static absolute top-2.5 right-2.5 vl:col-start-5 flex items-center"
