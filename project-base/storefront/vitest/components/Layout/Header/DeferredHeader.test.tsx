...
describe('deferred desktop header', () => {
    test('keeps placeholders before the viewport is recognized even after the deferred wave', () => {
        render(<HeaderParts isDesktop={undefined} />);
    ...
    test('removes the search placeholder once the mobile viewport is recognized', () => {
        const { rerender } = render(<DeferredAutocompleteSearch isDesktop={undefined} />);

        expect(screen.getByText('Search placeholder')).toBeInTheDocument();

        rerender(<DeferredAutocompleteSearch isDesktop={false} />);
