...
use Shopsys\LuigisBoxBundle\Model\Product\Filter\Exception\ValueIsNotSliderFormatException;
...
class LuigisBoxFacetsToProductFilterOptionsMapper
{
    ...
    protected function mapPriceRange(array $facetData): PriceRange
    {
        ...
        return new PriceRange(Money::create($sliderValues['minimalValue']), Money::create($sliderValues['maximalValue']));
    }
    ...
    protected function mapParameterWithValues(
        ...
        if ($isSlider) {
            try {
                $sliderValues = $this->getMinimalAndMaximalValueForSlider($facetData);
                $parameterValues = $this->parameterValueRepository->getSliderParameterValuesBetweenMinAndMaxByLocale($parameter, $sliderValues['minimalValue'], $sliderValues['maximalValue'], $this->domain->getLocale());
    ...
    protected function getMinimalAndMaximalValueForSlider(array $facetData): array
    {
        ...
        foreach ($facetData['values'] as $facetValue) {
            ...
            if ($minValue < $minimalValue || $minimalValue === 0) {
