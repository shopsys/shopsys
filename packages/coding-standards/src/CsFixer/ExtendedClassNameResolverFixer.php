...
final class ExtendedClassNameResolverFixer implements ConfigurableFixerInterface
{
    ...
    private const string RESOLVER_CLASS_NAME = ExtendedClassNameResolver::class;
...
SAMPLE,
    ...
    private function wrapClassName(Tokens $tokens, int $classNameStartIndex, int $classNameEndIndex): void
    {
        ...
        $tokens->insertAt($classNameStartIndex, [
            new Token([T_STRING, $this->getResolverShortName()]),
            new Token([T_DOUBLE_COLON, '::']),
    ...
    private function getResolverShortName(): string
    {
        return substr(strrchr(self::RESOLVER_CLASS_NAME, '\\'), 1);
    }
    ...
    private function addResolverImport(
        ...
        if ($namespace->getFullName() === substr(self::RESOLVER_CLASS_NAME, 0, -strlen('\\' . $this->getResolverShortName()))) {
            return;
        }
