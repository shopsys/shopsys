...
class LoginController extends AbstractConvertimController
{
    ...
    public function login(Request $request): Response
    {
        ...
        try {
            ...
            $productListUuidsCookie = $request->cookies->get('productListUuids');
            $productListUuids = [];

            if (is_string($productListUuidsCookie)) {
                try {
                    $productListUuids = json_decode($productListUuidsCookie, true, 512, JSON_THROW_ON_ERROR);
                } catch (JsonException) {
                }
            }
            ...
            $loginResultData = $this->loginAsUserFacade->runLoginSteps(
                ...
                LoginTypeEnum::WEB,
                false,
                is_array($productListUuids) ? $productListUuids : [],
                true,
                $cartUuid,
                null,
            );
