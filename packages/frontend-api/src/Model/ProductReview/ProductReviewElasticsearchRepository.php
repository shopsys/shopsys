...
class ProductReviewElasticsearchRepository
{
    ...
    /**
     * Reads a single page of the reviews stored on the document of the main variant,
    ...
    public function getReviewsPage(
        ...
        $productArray = $this->extractProductArray($result, sprintf('Product with UUID %s does not exist.', $productUuid));

        if ($productArray[ProductExportFieldProvider::IS_VARIANT] === true
            && $productArray[ProductExportFieldProvider::MAIN_VARIANT_ID] !== null
        ) {
            $result = $this->search(
                $this->filterQueryFactory->createVisibleProductsByProductIdsFilter([$productArray[ProductExportFieldProvider::MAIN_VARIANT_ID]]),
                $orderingMode,
                $offset,
                $limit,
            );

            $productArray = $this->extractProductArray($result, sprintf('The main variant of the product with UUID %s does not exist.', $productUuid));
        }
