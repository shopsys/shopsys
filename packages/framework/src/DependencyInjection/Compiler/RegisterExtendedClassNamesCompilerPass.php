...
final class RegisterExtendedClassNamesCompilerPass implements CompilerPassInterface
{
    ...
    public function __construct(
        private readonly string $shopsysVendorNamespacePrefix = 'Shopsys\\',
    ) {
    }
    ...
    private function getServiceExtensionMap(ContainerBuilder $container): array
    {
        ...
        foreach ($container->getServiceIds() as $serviceId) {
            if (!$this->isShopsysVendorClass($serviceId)) {
                continue;
            }
            ...
            if ($isInterface && $this->isShopsysVendorClass($resolvedClassName)) {
                continue;
            }
    ...
    private function getInterfaceVariantMap(string $serviceId, string $resolvedClassName, bool $isInterface): array
    {
        ...
        if ($this->isShopsysVendorClass($resolvedClassName)) {
            return [];
        }
    ...
    private function isShopsysVendorClass(string $className): bool
    {
        return str_starts_with($className, $this->shopsysVendorNamespacePrefix);
    }
}
