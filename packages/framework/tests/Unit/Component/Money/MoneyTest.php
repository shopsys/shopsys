...
class MoneyTest extends TestCase
{
    ...
    public function testFromString(string $string, string $expected): void
    {
        ...
        $this->assertEquals($expected, $money->toString());
    }
    ...
    {
        $this->expectException(BigNumbersError::class);
    ...
    public function testAddIsImmutable(): void
    {
        ...
        $this->assertEquals('1', $money->toString());
    }
    ...
    public function testModIsImmutable(): void
    {
        $money = Money::fromString('1');

        $money->mod(Money::fromString('1'));
     ...
     * @param string $expectedResult
     */
    public function testMod(string $a, string $b, string $expectedResult): void
    {
        $moneyA = Money::fromString($a);
        $moneyB = Money::fromString($b);

        $result = $moneyA->mod($moneyB);
     ...
     * @return \Iterator
     */
    public function modProvider(): Iterator
    {
        yield ['100', '100', '0'];
        yield ['120', '100', '20'];
        yield ['530', '100', '30'];
        yield ['-20', '100', '80'];
        yield ['-530', '100', '70'];
        yield ['0.001', '100', '0.001'];
        yield ['500.001', '100', '0.001'];
        yield ['0.75', '0.5', '0.25'];
    }
    ...
    public function testRoundIsImmutable(): void
    {
        ...
        $this->assertEquals('1.5', $money->toString());
    }
     ...
     * @param int $scale
     */
    public function testRoundingIsSameAsInPhp(string $amount, int $scale): void
    {
        $money = Money::fromString($amount);

        $result = $money->round($scale);
        $expectedResult = round($amount, $scale);
    ...
    {
        foreach ($this->compareProvider() as $dataset) {
            yield[$dataset[0], $dataset[1], $dataset[2] === 0];
        }
    }
