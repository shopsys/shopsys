...
class SeoGroupTypeTest extends TypeTestCase
{
    ...
    public function testSubmittedDataAreWrittenPerDomainOfMultidomainEntity(): void
    {
            ...
            'seoGroup' => [
                'domains' => [
                    (string)Domain::SECOND_DOMAIN_ID => [
                        ...
                        'urls' => ['newUrls' => ['second-domain-slug']],
                    ],
                ],
            ],
        ]);
        ...
        $this->assertSame(['second-domain-slug'], $brandData->urls[Domain::SECOND_DOMAIN_ID]->newUrls);
    ...
    public function testSubmittedDataAreWrittenForSingleDomainEntity(): void
    {
        ...
        $this->assertSame(['article-slug'], $articleData->urls->newUrls);
    }
    ...
    {
        $domainRouterFactory = $this->createDomainRouterFactoryMatchingNoRoute();

        return [
            $this->createValidatorExtensionWithUniqueSlugsOnDomain($this->domain, $domainRouterFactory),
            new PreloadedExtension(
                [
                    ...
                    $this->createUrlListType($this->domain, $domainRouterFactory),
                ],
                [],
            ),
        ];
