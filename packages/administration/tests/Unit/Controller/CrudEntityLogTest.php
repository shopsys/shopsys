...
final class CrudEntityLogTest extends TestCase
{
    ...
    {
        $crudController = $this->createCrudController(new CrudConfig('Product review'), isEntityLoggable: true);

        $this->assertTrue($crudController->isEntityLogShownForTest());
    }

    public function testEntityLogHiddenByConfigIsNotShown(): void
    {
        $crudConfig = new CrudConfig('Product review');
        $crudConfig->showEntityLog(false);
        $crudController = $this->createCrudController($crudConfig, isEntityLoggable: true);
    ...
    {
        $crudController = $this->createCrudController(new CrudConfig('Product review'), isEntityLoggable: false);

        $this->assertFalse($crudController->isEntityLogShownForTest());
    }

    private function createCrudController(CrudConfig $crudConfig, bool $isEntityLoggable): EntityLogTestCrudController
    {
