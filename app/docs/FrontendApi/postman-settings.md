...
Postman is strong tool for working with different APIs.
...
### Download schema.graphql from CI server

Open this project in `Gitlab` and go to `CI / CD` -> `Pipelines` section.
Find last build of desired branch and open it.
...
Enter desired API name like `Shopsys Commerce Cloud Frontend API`.
Version number is up to you as we are not versioning Frontend API.
...
Enter desired collection name like `Shopsys Commerce Cloud Frontend API`.
