    ...
    CKEDITOR.plugins.add('sharedspace', {
        init: function (editor) {
            ...
            editor.on('loaded', function () {
                ...
                if (spaces) {
                    for (var spaceName in spaces) { create(editor, spaceName, spaces[ spaceName ]); }
                }
            }, null, null, 9);
        }
    });
        ...
        if (target) {
            ...
            if (innerHtml) {
                ...
                editor.on('uiSpace', function (ev) {
                    if (ev.data.space == spaceName) { ev.cancel(); }
                ...
                if (target.getCustomData('cke_hasshared')) { space.hide(); } else { target.setCustomData('cke_hasshared', 1); }
                ...
                editor.on('focus', function () {
                    for (var i = 0, sibling, children = target.getChildren(); (sibling = children.getItem(i)); i++) {
                        if (sibling.type == CKEDITOR.NODE_ELEMENT && !sibling.equals(space) && sibling.hasClass('cke_shared')) {
                            sibling.hide();
                        }
                    }
