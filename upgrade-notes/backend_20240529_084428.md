...
-   `Shopsys\FrameworkBundle\Controller\Admin\CustomerController` class was changed:
    ...
    -   `loginAsUserAction()` method was removed, use `loginAsCustomerUserAction` instead
...
-   `Shopsys\FrameworkBundle\Model\Mail\MailTemplateRepository` class was changed:
    -   `__construct()` changed its interface:
        ...
        ```
-   `Shopsys\FrontendApiBundle\Model\Mutation\Customer\User\CustomerUserMutation` class was chagned:
    -   `__contruct()` method changed its interface:
        ...
        ```
    -   `registerMutation()` - the returned array changed its structure to `{tokens: array{accessToken: string, refreshToken: string}, showCartMergeInfo: bool}`
-   `Shopsys\FrontendApiBundle\Model\Order\OrderApiFacade::__contruct()` changed its interface:
    ```diff
        public function __construct(
            // ...
    +       protected readonly EntityManagerInterface $em,
...
-   `Shopsys\FrontendApiBundle\Model\User\FrontendApiUser::__contruct()` changed its interface:
    ```diff
        public function __construct(
            // ...
    ...
    ```
-   [features moved](#movement-of-features-from-project-base-to-packages) from project-base to the framework package:
    -   `CustomerController::loginAsCustomerUserAction()` and all the related logic (see `LoginAsUserFacade`)
    ...
    -   `CustomerUserUpdateDataFactory` methods:
        -   `createAmendedByOrder()`
