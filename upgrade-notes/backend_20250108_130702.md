...
- `Shopsys\FrameworkBundle\Model\Product\Pricing\ProductSellingPrice` was removed, use `Shopsys\FrameworkBundle\Model\Product\Pricing\ProductPrice` instead
    ...
    - `PricesWithCalculatedSellingPricesType` was renamed to `PricesWithCalculatedPricesType` and its option `selling_prices` was renamed to `product_prices`
    - `MoneyWithCalculatedPriceType` option `selling_price` was renamed to `product_price`
