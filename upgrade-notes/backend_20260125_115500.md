...
- new coding standards are enforced by `SlevomatCodingStandard\Sniffs\TypeHints\ParameterTypeHintSniff` and `SlevomatCodingStandard\Sniffs\TypeHints\ReturnTypeHintSniff`
    ...
    - you can try cherry-picking relevant commits from `project-base` repository that could help you to fix the compilation errors caused by the parent-child class incompatibilities <!--- TODO add links to the relevant commits from the split pull request here -->
