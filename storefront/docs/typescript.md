...
```
#### Styling the app with TypeScript and StyledComponents
- when styling the app with StyledComponents, you can take advantage of TypeScript features but it requires some aditional typings

**Extending the DefaultTheme interface**
- first you have to extend the DefaultTheme interface in the @types/styled-components folder
- there you can declare the styled-components module from which you export the DefaultTheme interface:
```plain
...
```
- after that, you can use the DefaultTheme interface to type your theme object
```plain
...
- by default, each styled component receives props which contains the theme definition
- if you want to pass some custom props, you have to specify the props in a similar way you would specify props for a React component
``` plain
type StyledMyComponentProps = {
    state: "error" | "success";
};

...

export const StyledMyComponent = styled.div<StyledMyComponentProps>`...`