...
export const shallowReplaceIfDifferent: (router: NextRouter, ...parameters: Parameters<typeof router.replace>) => void =
    (router, url, as, options = { scroll: false }) => {
        if (
            typeof url !== 'string' &&
            areUrlsDifferent({ pathname: router.asPath.split('?')[0], query: getQueryWithoutAllParameter(router) }, url)
        ) {
