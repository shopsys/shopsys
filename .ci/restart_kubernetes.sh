...
kubectl exec --namespace=${JOB_NAME} ${PHP_FPM_POD} -- ./phing -D production.confirm.action=y clean dirs-create backend-api-install backend-api-oauth-keys-generate product-search-recreate-structure product-search-export-products grunt error-pages-generate
