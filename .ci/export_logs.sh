...
for POD_NAME in $(kubectl get pods -n ${JOB_NAME} | grep -v ^NAME | cut -f 1 -d ' '); do
    ...
    if [[ ${POD_NAME} == *${WEBSERVER_PHP_FPM_CONTAINER_NAME}* ]]; then
        ...
        if kubectl -n ${JOB_NAME} exec ${POD_NAME} -- test -d project-base/app/var/log/codeception; then
            kubectl cp ${JOB_NAME}/${POD_NAME}:/var/www/html/project-base/app/var/log/codeception/ ${WORKSPACE}/logs/codeception
        fi
    fi
done
