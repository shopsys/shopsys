...
SEO specialisté chtějí u **každé stránky s vlastní/friendly URL** jednotně nastavovat _page title_, _meta description_, _meta robots_ a _canonical URL_ (multidoménově, nepovinně) a mít jistotu, že storefront tyto hodnoty vyrenderuje už v SSR, že sitemapa respektuje `noindex`/canonical a že homepage i statické stránky se řídí stejným mechanismem (SEO pages) jako entity. Součástí je odstranění duplicitního nastavení „Headline / Meta description“ ze _Settings > SEO > SEO_ a doseedování SEO pages pro všechny nedynamické stránky storefrontu.
...
- Robots: komponenta `MetaRobots.tsx` na 24 místech (cart, search `noindex, nofollow`, order/_, customer/_, personal-data-_, new-password, order-detail, order-withdrawal_; category/brand/flag `noindex, follow` při `filter || sort`; blog draft/preview/future `noindex, nofollow`).
...
5. _Settings > SEO > SEO_ jen _Complement to title_; hodnoty přeneseny do SeoPage `/`.
6. Čistá instalace má SeoPage pro všechny uživatelské statické stránky (en/cs/sk), u neindexovatelných `noindex`.

### Key Discoveries

- Embeddable sloupce: `#[ORM\Embedded(class: SeoAttributes::class)] protected $seo;` → `seo_title`, `seo_meta_description`, `seo_h1` — existující data bez migrace (kromě SeoPage `canonical_url` a SeoMix přejmenování). `columnPrefix` netřeba: underscore naming strategy odvodí prefix z názvu property `$seo` (rozhodnuto 2026-09-02, ověřeno `db-check-mapping`).
...
| vrstva        | tvar                                                                                                                                                                                                                                                                                                         |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Entity        | `Shopsys\FrameworkBundle\Model\Seo\SeoAttributes` (`#[ORM\Embeddable]`): `title`, `metaDescription`, `h1`, `metaRobots`, `canonicalUrl`; v `*Domain`/single-domain entitě `#[ORM\Embedded(class: SeoAttributes::class)] protected $seo` (prefix `seo_` plyne z názvu property)                                                                             |
...
- [x] `docker compose exec php-fpm php phing clean db-check-mapping annotations-check phpstan tests-unit` prochází s embeddable na `BrandDomain`
- [x] `BrandDomainTest` prochází
...
- **Implementační odchylky od snippetů výše (finálně rozhodnuto 2026-09-03, snippety ponechány historicky):** generický kanál `options_by_field_name`/`seo_attributes_options`/`options_by_domain_id` **neexistuje** — jediné reálné použití bylo povinné H1 u SeoMixu, ostatní potřeby pokryly dedikované options. `SeoAttributesFormType` i `SeoGroupType` mají: `placeholder_source_input_id` (id inputu se jménem entity; formulář z něj dá `data-js-placeholder-source-input-id` na title+h1 — serverový `placeholder` se nepředává, `DynamicPlaceholder.js` si ho při initu kopíruje ze zdroje; pro lokalizované inputy token `{locale}` — `SeoGroupType` ho per doména nahradí locale, např. Product `'product_form_name_{locale}'`, Category `'category_form_settings_name_{locale}'`; pro případné per-doménové inputy existuje i token `{domain_id}`; popisy options jsou v `OptionsResolver::setInfo()`) a `h1_required` (SeoMix: required + NotBlank na h1). Dnešní `Length(max: 255)` na H1 u Category/BlogCategory/BlogArticle se **vypouští** — všechna SEO pole všech entit jsou v DB `text` bez limitu, pravidlo nemělo oporu (sjednocení = žádný limit nikde). `SeoGroupType` je doménově-aware vrstva (injektuje `Domain`, skládá `options_by_domain_id` pro `MultidomainType`), `SeoAttributesFormType` zůstává bez služeb. Ověřeno auditem všech 10 formulářů (2026-09-03): žádné jméno/placeholder není per doména (jen per locale → token `{locale}`; jediný per-doménový placeholder byl metaDesc ze Settings, který se ruší); Article předává `row_attr` (skrývání dle typu článku) — standardní option, prochází; SeoPage si og pole + og image přidá jako další children do group builderu (create+add). Phase 2 doplní interní `domain_id` (multidomain: `options_by_domain_id` spočítané uvnitř `SeoGroupType`; single-domain: nová option), z něhož si `SeoAttributesFormType` sám odvodí `data-js-canonical-domain-url`. Downstream úpravy = standardní form type extension na `SeoAttributesFormType`, žádné konfigurační API. Pokryto testem `packages/framework/tests/Unit/Form/Admin/Seo/SeoGroupTypeTest.php`.
- **Zjištění (2026-09-03, jednodoménový eshop):** (1) ~~Option `multidomain` svádí k omylu~~ **VYŘEŠENO 2026-09-04:** option `multidomain` už neexistuje — přepínačem je `domain_id` (`null` = SEO atributy pro všechny domény, id = jediná doména entity; z něj se odvozuje i canonical cross-domain varování). (2) ZBÝVÁ — UX: na eshopu s jedinou admin doménou je doménová karta („Domain B2C") v SEO skupině šum — šablona `seo_group_widget` (příp. i `url_list_widget`) by při právě jedné doméně mohla vynechat card chrome a renderovat pole naplocho.
...
// SeoMix: ['multidomain' => false, 'h1_required' => true, 'url_list_options' => [...]]
```
...
Přidání `seo_meta_robots`/`seo_canonical_url` do všech tabulek dělá Phase 2. **Rozhodnuto při implementaci (2026-09-02):** embeddable má v Phase 1 jen 3 pole (title, metaDescription, h1) — mapují se 1:1 na existující sloupce, takže per-entity commity nepotřebují migraci (kromě přejmenování u ReadyCategorySeoMix). Pole `metaRobots`/`canonicalUrl` přidá do embeddablu, `SeoAttributesData` i formuláře až Phase 2 společně s jednou migrací pro všechny tabulky. Důsledek: **SeoPage přechází na embeddable až po Phase 2** (jediná entita s existujícím `canonicalUrl` — přejmenování `canonical_url` → `seo_canonical_url` proběhne při jejím přechodu).
...
## Phase 1b: UrlListType v doménovém layoutu (přidáno 2026-09-02)

### Overview

`SeoGroupType` seskupil SEO pole do karty per doména (dřív: pole → v něm řádky domén). URL adresy do tohoto layoutu musí zapadnout: **tlačítko „Add URL“ v kartě každé domény** místo jednoho globálního, nový řádek se přidá do karty své domény s doménou implicitní — **selectbox domény ve `FriendlyUrlType` zmizí**. Samostatný commit, nezávislý na per-entity přechodech (zlepší všech 10 formulářů najednou).

### Changes Required

- `packages/administration/templates/form/type/UrlListType.html.twig` — doménové karty renderovat pro **všechny** povolené domény (`limit_domains_by_ids`), ne jen pro ty s existujícími URL (`form.toDelete`) — u nové entity dnes nejsou karty žádné; „Add URL“ tlačítko + kontejner nových URL do každé karty; prototype nové URL musí dostat `domainId` dané karty.
- `packages/framework/src/Form/FriendlyUrlType.php` — domain `ChoiceType` → `HiddenType` (hodnotu plní JS z karty domény); zvážit rovnou strukturu `newUrls` indexovanou doménou v `UrlListType.php` (pak hidden pole netřeba) — rozhodnout při implementaci podle toho, co méně rozbije `UniqueSlugsOnDomains` validaci a mapování na `UrlListData::$newUrls`.
- `packages/framework/assets/js/admin/components/EntityUrlsNew.js` — add-button per doména, doplnění domain id do prototypu, indexace nových řádků per doména.
- Ověřit dopad na `UrlListData`, `FriendlyUrlFacade::saveUrlListFormData()` a validátor `UniqueSlugsOnDomainsValidator` (struktura `newUrls`).

### Success Criteria

#### Automated Verification

- [x] `phing standards-fix phpstan tests-unit`, `phing npm` (admin assety se zbuildí) + nový funkční `UrlListTypeTest` (hidden domain s int transformerem, Choice restrikce na povolené domény, duplicitní slug)

#### Manual Verification

- [ ] Nová URL jde přidat v kartě konkrétní domény bez výběru domény; uloží se na správnou doménu
- [ ] Mazání/výběr hlavní URL beze změny; validace duplicitního slugu na doméně funguje
- [ ] Entita bez existujících URL (nový brand) zobrazuje karty všech domén s „Add URL“
...
Formuláře předají `data-js-canonical-domain-url` (`$domainConfig->getUrl()`) přes `options_by_field_name['canonicalUrl']['attr']` — multidoménově v `options_by_domain_id[$domainId]`, single-domain v `seo_attributes_options`. 4. **Admin JS** `packages/administration/assets/src/js/utils/canonicalUrlCrossDomainWarning.js` (vzor `recommendedLength.js`): na `input` u `input[data-js-canonical-domain-url]` porovnat `new URL(value).host` s hostem atributu, zobrazit/skrýt `<span class="text-warning small ms-3">` s `Translator.trans('This is a cross-domain canonical URL – it points to a different domain than the one being edited.')`; registrace přes `Register`, import v `packages/administration/assets/src/js/index.js:9-12`. 5. **Fallback description** — `TransformStringHelper::truncateToWholeWords(string $text, int $maxLength): string` (mb\_\*, řez na poslední mezeře, bez ellipsis, ořez trailing interpunkce) a v `SeoAttributesResultFactory`:
...
- **Hotovo předem (2026-09-04):** `url_list_widget` už doménové karty nekreslí (ploché slices přes makro `domainUrls`, chrome je vždy na volajícím — u multidomain entit ho kreslí `seo_group_widget` sloučenými kartami, single-domain jde přes `form_row`). Dočasný dopad: Store a Flag renderují URL adresy bez doménového oddělení, dokud tato fáze nepřepne jejich formuláře na `SeoGroupType` (vědomě akceptováno, commity lze v historii přeskládat). U Flagu tato fáze vrátí doménové členění sloučenými kartami SEO skupiny.
- **Room for improvement:** `limit_domains_by_ids` v `url_list_options` je pro SEO skupiny s `domain_id` redundantní — `SeoGroupType` ho odvozuje sám (default `[domain_id]`, volající ho nepředávají). Až Store/Flag přejdou na `SeoGroupType`, zvážit, zda `limit_domains_by_ids` z veřejného API `UrlListType` úplně odstranit — brání tomu jen samostatná použitelnost typu.
...
- [ ] _Settings > SEO > SEO_ jen „Complement to title“; required-settings lišta bez SEO hlášky; homepage `<title>` = SeoPage `/` + suffix
...
- [ ] _Settings > SEO > SEO pages_ obsahuje všechny stránky z tabulky per doména se správnými slugy
...
- **`docs/model/entities.md`** — příklad multidomain atributů je postavený na `BrandDomain::$seoTitle` + `BrandData::$seoTitles`/`$seoMetaDescriptions`/`$seoH1s` (cca řádky 90–340): aktualizovat na embeddable `SeoAttributes` (`$seo` + `BrandData::$seo`), včetně ukázek getterů/proxy metod na `Brand`. Přitom projít i další zmínky plochých SEO polí v docs: `docs/frontend-api/introduction-to-frontend-api.md:235-238` (resolver map příklad se `seoH1`/`seoTitle` — po GraphQL fázi zastaralé), `docs/administration/javascript-utilities.md:217` (příklad s `name="seoTitle"` — jen ilustrační, zvážit), `docs/integration/luigis-box.md:73` (zůstává platné — feed čte přes delegáty).
