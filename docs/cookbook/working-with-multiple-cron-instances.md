...
When you register a cron job in your configuration with tags mandatory for cron module, you can add the optional tag `instanceName`,
...
And because we have several cron instances registered, a job asks what cron instance should be run.
...
## Pitfalls
- If you tag cron module with another instance, without changes in Phing targets, your jobs will not be executed automatically because the command will hold on instance choice question.
- You can set your system to run too many cron jobs at once, resulting in server response time slowdown.
