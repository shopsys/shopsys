...
### Step 1 - Add `$extId` to [Product](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Shopsys/ShopBundle/Model/Product/Product.php) [entity](../model/introduction-to-model-architecture.md#entity)
...
#### 2.1 - Add new `ImportProductsCronModule` class that implements [`SimpleCronModuleInterface`](https://github.com/shopsys/shopsys/blob/8.0/packages/plugin-interface/src/Cron/SimpleCronModuleInterface.php)
...
#### 2.2 - Add cron configuration to [`cron.yml`](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Shopsys/ShopBundle/Resources/config/services/cron.yml)
```diff
...
!!! note
    ...
    For that purpose, we will use a descendant of [`ProductFacade`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductFacade.php) ([more about facades](../model/introduction-to-model-architecture.md#facade))
    which will use a descendant of [`ProductRepository`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductRepository.php) ([more about repositories](../model/introduction-to-model-architecture.md#repository))
...
#### 3.2 - Extend [`ProductRepository`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductRepository.php) and implement method `findByExternalId()` in order to be able find a [`Product`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/Product.php) by an external ID
...
Add information about the class extension into the container configuration in [`services.yml`](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Shopsys/ShopBundle/Resources/config/services.yml).
To [make the service public in TEST environment](../introduction/faq-and-common-issues.md#what-is-the-configuration-file-services_testyml-good-for), you need to add the same line into [`services_test.yml`](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Shopsys/ShopBundle/Resources/config/services_test.yml) as well.

```yaml
Shopsys\FrameworkBundle\Model\Product\ProductRepository: '@Shopsys\ShopBundle\Model\Product\ProductRepository'
```

#### 3.3 - Extend [`ProductFacade`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductFacade.php) and implement method `findByExternalId()` in order to get [`Product`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/Product.php) from repository
...
Those methods expect [`ProductData`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductData.php)
class as a parameter, you can use [`ProductDataFactory`](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Model/Product/ProductDataFactory.php) to create it.
...
[`ProductData`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductData.php) with external source data.
...
!!! note
    Data from external source contain only integer value for vat percent information but we need [`Vat`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/Vat.php) object
    in [`ProductData`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Product/ProductData.php).
    So we will extend [`VatRepository`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/VatRepository.php)
    and [`VatFacade`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/VatFacade.php) and implement appropriate methods.
...
#### 3.6 - Extend [`VatRepository`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/VatRepository.php) and implement method `getVatByPercent()` in order to load [`Vat`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/Vat.php) by percent
...
!!! danger
    The method throws an exception when [`Vat`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/Vat.php) object is not found by given percent value.
    Do not forget to handle it (e.g. skip the product data processing and log the exception).

#### 3.7 - Extend [`VatFacade`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/VatFacade.php) and implement method `getVatByPercent()` in it
...
- Use database transactions.
    ...
    - [`IteratedCronModuleInterface`](https://github.com/shopsys/shopsys/blob/8.0/packages/plugin-interface/src/Cron/IteratedCronModuleInterface.php) offers a way to implement longer processes that can span over many iterations.
- Disable entity manager SQL logging (use [`SqlLoggerFacade::temporarilyDisableLogging()`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Component/Doctrine/SqlLoggerFacade.php)).
    ...
    - Do not forget to re-enable the logging after the processing is done (use [`SqlLoggerFacade::reenableLogging()`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Component/Doctrine/SqlLoggerFacade.php)).
...
(e.g. [`Vat`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/src/Model/Pricing/Vat/Vat.php) object based on vat percent).
