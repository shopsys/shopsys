...
Sometimes you need to find all domain entities programmatically (eg. in [`CreateDomainsDataCommand`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Command/CreateDomainsDataCommand.php)).
You can use the [`MultidomainEntityClassFinderFacade`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Component/Domain/Multidomain/MultidomainEntityClassFinderFacade.php) which searches for all registered entities that have a composite identifier including a `domainId` field.
Exceptions (both for including and excluding particular class) can be provided via an implementation of [`MultidomainEntityClassProviderInterface`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Component/Domain/Multidomain/MultidomainEntityClassProviderInterface.php).
You should provide your own implementation if you need to alter the list of domain entities (otherwise, [`MultidomainEntityClassProvider`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/MultidomainEntityClassProvider.php) will be used).
...
(eg. [`OrderData`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Order/OrderData.php), method `getNewItemsWithoutTransportAndPayment`).
...
If you care about array keys, use a name of the key in the property name in form `propertyByKey`. Eg. [`TransportData::$pricesByCurrencyId`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Transport/TransportData.php).
...
If you need to transfer one entity, use PHPDoc annotation `entity|null`, eg. `\Shopsys\FrameworkBundle\Model\Pricing\Vat\Vat|null` in [`TransportData::$vat`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Transport/TransportData.php).
If you need to transfer a collection of entities, use PHPDoc annotation `entity[]` and initialize the array in the constructor, eg. `\Shopsys\FrameworkBundle\Model\Payment\Payment[]` in [`TransportData::$payments`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Transport/TransportData.php).
...
For boolean multidomain properties, we recommend using default value filled in the factory and PHPDoc annotation `bool[]` only, eg. property [`TransportData::$enabled`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Transport/TransportData.php).
...
Eg. [`CustomerData`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Customer/CustomerData.php) or [`OrderData`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Order/OrderData.php) (contains `OrderItemData`).
...
(e.g. [`PaymentDataFactory`](https://github.com/shopsys/shopsys/blob/7.3/packages/framework/src/Model/Payment/PaymentDataFactory.php) fills default VAT for new payment objects).
