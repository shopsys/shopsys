...
Row actions are buttons that are displayed in the last column of the grid. They are used to perform actions on a single row.

## Configuration

Row actions are defined by the `Datagrid` class using the `rowActions()` method. The method returns an instance of the `DatagridRowActions` class that is used to configure row actions.
...
$rowActions = $datagrid->rowActions();
...
// Let's look about the methods that will help you with redirecting the user after clicking the 
...
$rowAction
    ...
    // Use this in case you need to create an link to an external page
