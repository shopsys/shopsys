...
This phing target is usually triggered as the part of other phing targets, because it requires the application in a certain state (eg. with configured domains and an existing database structure), see [`build.xml`](https://github.com/shopsys/shopsys/blob/8.0/packages/framework/build.xml).
...
[`services_test.yml`](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Shopsys/ShopBundle/Resources/config/services_test.yml)
is a service configuration file that is loaded in TEST environment in addition to
the standard configuration defined in [`services.yml`](https://github.com/shopsys/shopsys/blob/8.0/project-base/src/Shopsys/ShopBundle/Resources/config/services.yml) as sometimes the configuration differs from the standard one and we need to override it.
