...
In the same time, we need to be able to improve Shopsys Platform for you by adding functionality, enhancing or simplifying current functions and fixing bugs.
...
It should be viewed as a template for your own ecommerce projects built on top of Shopsys Platform.

This means that the `project-base` should run with any higher minor version of Shopsys Platform, up to the next `MAJOR` version.
