...
- References to project files and classes have to be absolute links to the GitHub in the proper version which is ensured by `github.link` variable, e.g., `[config/parameters_common.yaml]({{github.link}}/project-base/app/config/parameters_common.yaml)`.
