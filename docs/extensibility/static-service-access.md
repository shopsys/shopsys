...
1. The [`RegisterExtendedClassNamesCompilerPass`]({{github.link}}/packages/framework/src/DependencyInjection/Compiler/RegisterExtendedClassNamesCompilerPass.php) builds one map of framework classes to the project classes extending them.
   ...
   The same map drives the `annotations-fix` tooling, so both tools agree on what is extended.
...
In your project, a class you have already extended can be called directly through your `App\` class.
A framework class you have not extended keeps going through the resolver, so that a later extension is picked up without touching the call sites.
The `Shopsys/extended_class_name_resolver` fixer applies this rule to your code as well.
