...
For this case, you can use `FormExtensions` in the namespace `App/Form/Admin` that extends `Symfony\Component\Form\AbstractTypeExtension`, which has a function called `getExtendedTypes()`.
Implement this function and return an array containing `class` of `ProductFormType` and add your fields into the form.
