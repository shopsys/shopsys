...
getBuildExitCodeBasedOnJobResults() {
    ...
    TESTS_STOREFRONT_REGENERATE_RESULT=${10}
    UNIT_TESTS_STOREFRONT_RESULT=${11}

    if [[ "$BUILD_FORK_RESULT" == "success" ]]; then
        return 0
    fi

    if [[ "$STANDARDS_RESULT" == "success" && "$TESTS_RESULT" == "success" && "$TESTS_ACCEPTANCE_RESULT" == "success" && "$STANDARDS_STOREFRONT_RESULT" == "success" && "$TRANSLATIONS_DUMP_RESULT" == "success" && "$REVIEW_RESULT" == "success" && "$CHECK_CONSOLE_COMMANDS_RESULT" == "success" && ("$TESTS_STOREFRONT_ACCEPTANCE_RESULT" == "success" || "$TESTS_STOREFRONT_REGENERATE_RESULT" == "success") && "$UNIT_TESTS_STOREFRONT_RESULT" == "success" ]]; then
        return 0
    else
        return 1
    fi
